...
// Generate social image for documentation pages
tasks.register("generateDocsSocialImage") {
    group = "documentation"
    description = "Generates social image for documentation pages"

    val outputFile = file("pages/assets/social-image.png")
    val htmlTemplate = file("pages/social-image-template.html")
    
    outputs.file(outputFile)
    inputs.file(htmlTemplate)
    ...
    doLast {
        ...
        val socialImageDir = file("build/socialImage")
        socialImageDir.mkdirs()
        
        // Copy template to intermediate location
        val intermediateTemplate = File(socialImageDir, "docs-social-image-template.html")
        htmlTemplate.copyTo(intermediateTemplate, overwrite = true)
        
        build_logic.generateSocialImageWithPlaywright(
            htmlTemplate = intermediateTemplate,
            outputFile = outputFile
        )
        println("Generated docs social image at ${outputFile.absolutePath}")
    }
}

// Make dokkaHtml depend on generateDocsSocialImage
tasks.named("dokkaHtml") {
    dependsOn("generateDocsSocialImage")
}
