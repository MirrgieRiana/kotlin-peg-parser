...
class ParserExtensionsTest {
    ...
    fun capturePropertyInSequence() {
        val parser =
            (+"hello").capture * (+" ").ignore * (+"world").capture map { (hello, world) ->
                "$hello-$world"
            }
    ...
    fun notPropertyInSequence() {
        ...
        val parser =
            (+"a").ignore * (+"b").not * (+"c").capture map { (c) ->
                "matched: $c"
            }
    ...
    fun stringPropertiesInSequence() {
        val parser =
            "hello".ignore * " ".ignore * "world".capture map { (world) ->
                world
            }
    ...
    fun charPropertiesInSequence() {
        val parser =
            'a'.ignore * 'b'.ignore * 'c'.capture map { (c) ->
                c
            }
    ...
    fun regexPropertiesInSequence() {
        val parser =
            Regex("[0-9]+").ignore * Regex("[a-z]+").capture map { (letters) ->
                letters.value
            }
